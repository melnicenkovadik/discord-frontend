...
const FriendsSidebar = () => {
  ...
  return (
    <div
      className="max-w-[300px] h-[100vh] bg-[#333c5a] flex flex-col px-3
    ...
    >
      ...
      "
      >
        {friendsWithOnlineStatus && friendsWithOnlineStatus?.length > 0 ? (
          friendsWithOnlineStatus.map(friend => {
            return <FriendsListItem key={friend.id} friend={friend} />;
          })
        ) : (
          <div
            className="flex items-center justify-center w-full min-h-[40px] mb-2
           hover:bg-darkGray rounded-md cursor-pointer px-2"
          >
