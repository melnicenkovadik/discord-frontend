...
export const FriendsView = () => {
  return (
    <>
      <Tabs value="all" className="w-[100%] h-[100%] flex flex-col justify-start items-center">
        <div className="w-[100%] flex justify-start items-start  h-[50px] bg-[#2f364f] px-2">
          <FriendsTabs />
        </div>
        <div className="p-[20px] flex flex-col w-[100%] justify-start items-start animate__animated animate__fadeInRightBig ">
          <TabsBody>
            {data.map(({ value }) => (
              <TabPanel key={value} value={value}>
                {value === 'add' ? <AddFriend /> : <FriendsBody value={value} />}
              </TabPanel>
            ))}
          </TabsBody>
        </div>
...
const FriendsTabs = () => {
  ...
  return (
    <div className="w-[100%] bg-transparent flex justify-start items-center">
      ...
      <TabsHeader className="w-[100%] bg-transparent flex justify-between items-center">
        ...
        <div className="w-[100%] bg-transparent flex justify-between items-center">
          <button
            className="h-[35px] flex-1 justify-center items-center text-gray flex shadow-xl"
...
const data = [
  ...
  {
    label: (
      <div className="flex whitespace-nowrap flex-row rounded-lg flex items-center justify-center cursor-pointer ">
        <div>
          <svg
...
export default function UsersList({ type }: { type: string }) {
  ...
  const { friends, onlineUsers, pendingFriendsInvitations } = useAppSelector(
    state => state.friends,
  );
  const [
    acceptInvitation,
      ...
      <div className="w-[100%] flex flex-col justify-start items-start  gap-1 p-3 px-2">
        {type === 'waiting' && pendingFriendsInvitations && pendingFriendsInvitations?.length > 0
          ? pendingFriendsInvitations.map((friend: any) => {
              return (
                <div
                  key={friend.senderId._id}
                  className="flex w-[100%] flex-row justify-between items-between
